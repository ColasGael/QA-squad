...
class SQuAD(data.Dataset):
    ...
    def __init__(self, data_path, use_v2=True):
        ...
        self.context_pos_idxs = torch.from_numpy(dataset['context_pos_idxs']).long() # NEW
        self.context_ner_idxs = torch.from_numpy(dataset['context_ner_idxs']).long() # NEW 
        self.context_ems = torch.from_numpy(dataset['context_ems']).long() # NEW 
        self.context_tfs = torch.from_numpy(dataset['context_tfs']) # NEW 
        self.question_idxs = torch.from_numpy(dataset['ques_idxs']).long()
        self.question_char_idxs = torch.from_numpy(dataset['ques_char_idxs']).long()
        self.question_pos_idxs = torch.from_numpy(dataset['ques_pos_idxs']).long() # NEW
        self.question_ner_idxs = torch.from_numpy(dataset['ques_ner_idxs']).long() # NEW
        self.question_ems = torch.from_numpy(dataset['ques_ems']).long() # NEW 
        self.question_tfs = torch.from_numpy(dataset['ques_tfs']) # NEW 
        ...
        if use_v2:
            ...
            # token 0 = not Exact Match
            zeros_int = torch.zeros((batch_size, 1), dtype=torch.int64) # NEW
            # token 0 = appears in every document
            zeros_float = torch.zeros((batch_size, 1), dtype=torch.float64) # NEW

            self.context_idxs = torch.cat((ones, self.context_idxs), dim=1)
            self.context_pos_idxs = torch.cat((ones, self.context_pos_idxs), dim=1) # NEW
            self.context_ner_idxs = torch.cat((ones, self.context_ner_idxs), dim=1) # NEW   
            self.context_ems = torch.cat((zeros_int, self.context_ems), dim=1) # NEW   
            self.context_tfs = torch.cat((zeros_float, self.context_tfs), dim=1) # NEW   
            
            self.question_idxs = torch.cat((ones, self.question_idxs), dim=1)
            self.question_pos_idxs = torch.cat((ones, self.question_pos_idxs), dim=1) # NEW
            self.question_ner_idxs = torch.cat((ones, self.question_ner_idxs), dim=1) # NEW
            self.question_ems = torch.cat((zeros_int, self.question_ems), dim=1) # NEW   
            self.question_tfs = torch.cat((zeros_float, self.question_tfs), dim=1) # NEW  
    ...
    def __getitem__(self, idx):
        ...
        example = (self.context_idxs[idx],
                   ...
                   self.context_pos_idxs[idx], # NEW
                   self.context_ner_idxs[idx], # NEW
                   self.context_ems[idx], # NEW
                   self.context_tfs[idx], # NEW
                   self.question_idxs[idx],
                   self.question_char_idxs[idx],
                   self.question_pos_idxs[idx], # NEW
                   self.question_ner_idxs[idx], # NEW
                   self.question_ems[idx], # NEW
                   self.question_tfs[idx], # NEW
                   self.y1s[idx],
                   self.y2s[idx],
                   self.ids[idx])
...
def collate_fn(examples):
    ...
    context_idxs, context_char_idxs, \
        context_pos_idxs, context_ner_idxs, \
        context_ems, context_tfs, \
        question_idxs, question_char_idxs, \
        question_pos_idxs, question_ner_idxs, \
        question_ems, question_tfs, \
        y1s, y2s, ids = zip(*examples)

    # Merge into batch tensors
    context_idxs = merge_1d(context_idxs)
    context_char_idxs = merge_2d(context_char_idxs)
    context_pos_idxs = merge_1d(context_pos_idxs) # NEW
    context_ner_idxs = merge_1d(context_ner_idxs) # NEW
    context_ems = merge_1d(context_ems, pad_value=-1) # NEW
    context_tfs = merge_1d(context_tfs, dtype=torch.float64, pad_value=-1) # NEW
    question_idxs = merge_1d(question_idxs)
    question_char_idxs = merge_2d(question_char_idxs)
    question_pos_idxs = merge_1d(question_pos_idxs) # NEW
    question_ner_idxs = merge_1d(question_ner_idxs) # NEW
    question_ems = merge_1d(question_ems, pad_value=-1) # NEW
    question_tfs = merge_1d(question_tfs, dtype=torch.float64, pad_value=-1) # NEW
    y1s = merge_0d(y1s)
    y2s = merge_0d(y2s)
    ids = merge_0d(ids)

    return (context_idxs, context_char_idxs,
            context_pos_idxs, context_ner_idxs, # NEW
            context_ems, context_tfs, # NEW
            question_idxs, question_char_idxs, 
            question_pos_idxs, question_ner_idxs, # NEW
            question_ems, question_tfs, # NEW
            y1s, y2s, ids)
...
def analysis_dicts(gold_dict, pred_dict, no_answer):
    avna = f1 = f1_a = em = pred_pos = true_pos = total = total_a = 0
    for key, value in pred_dict.items():
        total += 1
        ground_truths = gold_dict[key]['answers']
        prediction = value
        ...
        f1 += metric_max_over_ground_truths(compute_f1, prediction, ground_truths)
        if no_answer:
            # Answer versus No-Answer accuracy
            avna += compute_avna(prediction, ground_truths)
            # number of predicted no-answers
            pred_pos += 1 - 1*bool(prediction)
            
            # check if the question had an answer
            if bool(ground_truths):
                total_a += 1
                f1_a += metric_max_over_ground_truths(compute_f1, prediction, ground_truths)
            
            else:
                true_pos += compute_avna(prediction, ground_truths)
        

    analysis_dict = {'EM': 100. * em / total,
                 'F1': 100. * f1 / total}

    if no_answer:
        analysis_dict['AvNA'] = 100. * avna / total
        # Answer metrics
        analysis_dict['F1_A'] = 100. * f1_a / total_a
        # number of No-Answer examples
        total_na = total - total_a
        # No-Answer metrics
        analysis_dict['recall'] = recall = 100. * true_pos / total_na
        analysis_dict['precision'] = precision = 100. * true_pos / pred_pos
        analysis_dict['F1_NA'] = (2 * precision * recall) / (precision + recall)
