...
class SegmentSQuAD(data.Dataset):
    """Stanford Question Answering Dataset (SQuAD).
    ...
    Args:
        ...
        use_v2 (bool): Whether to use SQuAD 2.0 questions. Otherwise only use SQuAD 1.1.
    """
    def __init__(self, data_path, use_v2=True, win_len=40, stride=10):
        super(SegmentSQuAD, self).__init__()
        ...
        if use_v2:
            ...
            # Modification for the segmented version
            new_context_idxs = []
            for i in range(self.context_idxs.size(0)):
                new_context_idxs_i = []
                for j in range(0, self.context_idxs.size(1), stride):
                    if j > self.context_idxs.size(1) - 1:
                        break

                    if j + win_len < self.context_idxs.size(1) + 2:
                        new_context_idxs_i.append(self.context_idxs[i, j:j+win_len])
                    else:
                        break
                        #new_context_idxs_i.append(self.context_idxs[i, j:])
                new_context_idxs.append(torch.stack(new_context_idxs_i)) 
            self.context_idxs = torch.stack(new_context_idxs)
...
def collate_fn(examples):
    ...
    print(context_idxs[0].shape)
    if len(context_idxs[0].shape) == 1:
        context_idxs = merge_1d(context_idxs)
    elif len(context_idxs[0].shape) == 2:
