...
def get_one_zero_mat(batch_size, passage_lens, question_lens, max_passage_len, max_question_len):
    zero_one = np.ones((batch_size, max_passage_len, max_question_len))
    ...
    zero_one = zero_one.astype(np.float32)
    #zero_one = torch.tensor(zero_one, dtype=torch.float32).cuda()

    return zero_one


def get_zero_one_mat(batch_size, passage_lens, max_len):
    zero_one = np.ones((batch_size, max_len))
...
def get_ones_zeros_mat(batch_size, lengths, max_length_1, max_length_2):
    zero_one = np.ones((batch_size, max_length_1, max_length_2))

    for i in range(batch_size):
        if max_length_2 != lengths[i].item():
            zero_one[i][:, lengths[i].item():] = np.zeros((max_length_1, (max_length_2 - lengths[i].item())))
...
def get_ones_and_zeros_mat(batch_size, sequence_lengths, hidden_dim, max_len):
    zero_one = np.ones((batch_size, max_len, 2 * hidden_dim))

    for i in range(batch_size):
        if max_len != sequence_lengths[i].item():
            zero_one[i][sequence_lengths[i].item():, :] = np.zeros((
                (max_len - sequence_lengths[i].item()), 2 * hidden_dim))
